Feature: control multiple openflow switchies using sliceable_routing_switch
  ...
  Scenario: One openflow switch, two slices, four servers, mac binding ( loose mode )
    ...
    When I try trema run "../apps/sliceable_routing_switch/sliceable_routing_switch -s tmp/slice.db -f tmp/filter.db --loose" with following configuration (backgrounded):
      """
