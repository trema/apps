...
int
foreach_port( const list_element *ports, int ( *function )( port_info *port, void *user_data ), void *user_data ) {
  int r = 0;
  for ( const list_element *p = ports; p != NULL; p = p->next ) {
    r += ( *function )( p->data, user_data );
  }
...
void
foreach_switch( const list_element *switches, void ( *function )( switch_info *sw, void *user_data ), void *user_data ) {
  for ( const list_element *s = switches; s != NULL; s = s->next ) {
    ( *function )( s->data, user_data );
  }
}
