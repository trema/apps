...
bool
setup_path( path *path, setup_handler setup_callback, void *setup_user_data,
            teardown_handler teardown_callback, void *teardown_user_data ) {
  path_private *private = ( path_private * ) copy_path( path );
  uint64_t in_datapath_id = private->in_datapath_id;

  if ( lookup_path( in_datapath_id, path->match, path->priority ) != NULL ) {
